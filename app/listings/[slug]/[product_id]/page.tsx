    ...
    if (isCurrentlyLiked) {
      ...
      if (response.status === 400) {
        ...
        if (errorData.includes("You have not liked this listing")) {
          console.log(
            "Item was already not liked on the server, treating as success"
          );
...
export default function ListingDetailScreen() {
  ...
  const updateListingStatus = async (
    ...
    if (!isAuthenticated) {
      Alert.alert(
        ...
        "Please sign in to update listing status"
      );
  ...
  const toggleLike = async () => {
    if (!isAuthenticated) {
      Alert.alert(
        "Authentication Required",
        "Please sign in to like this listing"
      );
      router.push("/auth/signin");
      return;
    }
    ...
    try {
      ...
      console.log(
        `Successfully ${isLiked ? "unliked" : "liked"} listing ${product_id}`
      );
    } catch (error: unknown) {
      // Only reach this point if all retry attempts failed
      console.log(
        "All retry attempts failed when toggling like status:",
        error
      );
  ...
  const renderCarouselItem = ({ item }: { item: any; index: number }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          const index = arrangedImages.findIndex(
      ...
      >
        <View style={styles.imageContainer}>
          <Image
            ...
            onError={() =>
              console.log("Failed to load carousel image:", imageUrl)
            }
          />
        </View>
      </TouchableOpacity>
    ...
    return (
      ...
      >
        <Image
          ...
          placeholder={{ uri: getPlaceholderImage() }}
          onError={() =>
            console.log("Failed to load thumbnail image:", imageUrl)
          }
        />
      </TouchableOpacity>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity style={styles.likeButtonTopRight} onPress={toggleLike}>
            ...
            {arrangedImages.length > 1 && (
              ...
              >
                {arrangedImages.map((image, index) =>
                  renderThumbnailItem(image, index)
                )}
              </ScrollView>
            )}
          </View>
