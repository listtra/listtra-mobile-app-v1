...
export default function ListingEditScreen() {
    ...
    try {
      ...
      if (data.images && Array.isArray(data.images)) {
        ...
        const primaryImage = data.images.find(
          (img: { id: string; is_primary: boolean }) => img.is_primary === true
        );
        if (primaryImage) {
  ...
  const pickImages = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      ...
      selectionLimit:
        5 - (existingImages.length - imagesToDelete.length + newImages.length),
      quality: 1,
    });
          ...
          <GooglePlacesAutocomplete
            ...
            onPress={(data, details = null) => {
              if (details) {
                ...
                const locationString = [suburb, state, country, postalCode]
                  .filter(Boolean)
                  .join(", ");
                setFormData((prev) => ({ ...prev, location: locationString }));
              } else {
        ...
        <View style={styles.imageSection}>
                    ...
                    <View key={image.id} style={styles.imageContainer}>
                      <View style={styles.imageWrapper}>
                        <Image
                          ...
                          style={[styles.image, isMain && styles.mainImage]}
                        />
                        {isMain && (
                    ...
                    <View key={index} style={styles.imageContainer}>
                      <View style={styles.imageWrapper}>
                        <Image
                          ...
                          style={[styles.image, willBeMain && styles.mainImage]}
                        />
                        {willBeMain && (
                          <View style={styles.mainImageBadge}>
          ...
          <View style={styles.addImagesContainer}>
            ...
            <Text style={styles.addImagesNote}>
              Upload up to {getRemainingImageCount()} more images (max 5MB
              each).
            </Text>
          </View>
        </View>
