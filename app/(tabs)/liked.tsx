...
export default function LikedScreen() {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 401) {
          ...
          setError(
            error.response?.data?.detail || "Failed to fetch liked listings"
          );
        }
      } else {
  ...
  const renderListingItem = ({ item }: { item: Listing }) => {
    ...
    const isItemLiked =
      likedItems[item.product_id] !== undefined
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          const listingUrl = `https://listtra.com/listings/${
            item.slug || "item"
          }/${item.product_id}`;
